$(function() {
	...
	$.getJSON("../js/data.json",function(value) {
		...
		$(".shop_l>img").attr("src",value.deta_tu_l[0].tu_b[0].url);
		for(var i = 0;i < value.deta_tu_l[1].tu_s.length;i ++) {
			//console.log(value.deta_tu_l[1].tu_s[i].url);
			$("<a href='javascript:;'></a>").css({"background":"url(" + value.deta_tu_l[1].tu_s[i].url + ")","background-size":"60px 60px"}).appendTo($(".list_tu"));
		}
		...
		$(".list_tu a").mouseover(function() {
			...
			$(".shop_l>img").attr("src",value.deta_tu_l[0].tu_b[$(this).index()].url);
		})
		...
		$(".shop_c p:nth-child(4) a").css({"background-image":"url(../img/details/deta_crb.png),url(../img/details/deta_t2.jpg)","background-size":"11px 11px,50px 50px","background-position":"right bottom,left top","background-repeat":"no-repeat"});
		...
		for(var i = 0;i < value.con_c[0].d2_c.length;i ++) {
			$("<img />").attr("src",value.con_c[0].d2_c[i].url).appendTo($(".d2_c"));
		}
		for(var i = 0;i < value.con_c[1].d3_c.length - 1;i ++) {
			$("<img />").attr("src",value.con_c[1].d3_c[i].url).appendTo($(".d3_c"));
		}
			...
			if($.cookie("shop_" + $(".shop_c>p:nth-child(1)").text()) != null) {
				for(var i in $.cookie()) {
					if(i == "shop_" + $(".shop_c>p:nth-child(1)").text()) {
						if(eval('(' + $.cookie(i) + ')').name == $(".shop_c>p:nth-child(1)").text() && eval('(' + $.cookie(i) + ')').color == $(".color").attr("title") && eval('(' + $.cookie(i) + ')').size == $(".size").attr("title") && eval('(' + $.cookie(i) + ')').price == $(".f9 p b").text().replace("￥","")) {
							$.cookie("shop_" + $(".shop_c>p:nth-child(1)").text(),"{id:'" + $(".f6_l span").text() + "',name:'" + $(".shop_c>p:nth-child(1)").text() + "',color:'" + $(".color").attr("title") + "',size:'" + $(".size").attr("title") + "',price:'" + $(".f9 p b").text().replace("￥","") + "',num:'" + (Number($(".num").val()) + Number(eval('(' + $.cookie(i) + ')').num)) + "',url:" + $(".color").css("background-image").toString().split(",")[1].replace("url(","").replace(")","") + "}");
						}else {
							$.cookie("shop_" + $(".shop_c>p:nth-child(1)").text(),"{id:'" + $(".f6_l span").text() + "',name:'" + $(".shop_c>p:nth-child(1)").text() + "',color:'" + $(".color").attr("title") + "',size:'" + $(".size").attr("title") + "',price:'" + $(".f9 p b").text().replace("￥","") + "',num:'" + $(".num").val() + "',url:" + $(".color").css("background-image").toString().split(",")[1].replace("url(","").replace(")","") + "}");
						}
					}
				}
			}else {
